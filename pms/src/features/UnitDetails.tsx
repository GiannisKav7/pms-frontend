...
import { Descriptions, Tabs, Table, Button, Form, Divider, Select, InputNumber } from "antd";
...
export interface UnitDetailsType {
  ...
  weeklyRent: number;
  area: number;
  residentialBedrooms: number;
  residentialBathrooms: number;
  descriptionNotes: string;
}

const initialDetails: UnitDetailsType = {
...
const UnitDetails: React.FC = () => {
  ...
  const [form] = Form.useForm<UnitDetailsType>();
  const [unitDetails, setUnitDetails] = useState<UnitDetailsType>(initialDetails);
  ...
  const handleSave = (): void => {
    form.validateFields().then((values: UnitDetailsType) => {
