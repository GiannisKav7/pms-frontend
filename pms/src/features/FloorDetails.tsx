...
const FloorDetails: React.FC = () => {
  const initialDetails: FloorDetailsData = {
    propertyCode: "P-001",
    buildingCode: "B-001",
    floorCode: "F-001",
  ...
  const [form] = Form.useForm<FloorDetailsData>();
  const [details, setDetails] = useState<FloorDetailsData>(initialDetails);
  ...
  const handleNavigation = (code: string, path: string) => {
    navigate(`/${path}/${code}`);
  };
  ...
  const generalData = [
    {
      ...
      field3: "Custom Value 3",
      field4: "Custom Value 4",
      field5: "Custom Value 5",
      field6: "Custom Value 6",
    },
  ];
    ...
    units: [
      {
        ...
        key: "propertyCode",
        render: (text: string) => (
          <a style={{ color: "#1890ff" }} onClick={() => handleNavigation(text, "property")}>
            {text}
          </a>
        ),
      },
      {
        ...
        key: "unitTypeCode",
        render: (text: string) => (
          <a style={{ color: "#1890ff" }} onClick={() => handleNavigation(text, "unitType")}>
            {text}
          </a>
        ),
      },
  ...
  return (
    <div style={{ padding: 24, background: "#f9f9f9" }}>
      ...
      <Form form={form} layout="vertical">
        ...
        <Descriptions bordered column={2} style={{ background: "#fff", borderRadius: "8px" }}>
            ...
            >
              {key.toLowerCase().includes("code") && !isEditing ? (
                <a
                  style={{ color: "#1890ff" }}
                  onClick={() =>
                    handleNavigation(String(value), key.replace("Code", "").toLowerCase())
                  }
                >
                  {String(value)}
                </a>
                ...
                <Form.Item
                  name={key as keyof FloorDetailsData}
                  noStyle
