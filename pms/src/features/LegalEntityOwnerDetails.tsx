...
interface LegalEntityOwnerDetailsType {
...
const initialDetails: LegalEntityOwnerDetailsType = {
...
const LegalEntityOwnerDetails: React.FC = () => {
  const [isEditing, setIsEditing] = useState<boolean>(false);
  const [form] = Form.useForm<LegalEntityOwnerDetailsType>();
  const [details, setDetails] = useState<LegalEntityOwnerDetailsType>(initialDetails);
  ...
  const tabColumns = {
    ...
    taxInformation: [
      ...
      { title: "Initial Reporting Period", dataIndex: "initialReportingPeriod", key: "initialReportingPeriod" },
      {
        ...
        render: (text: any) => (
          <a style={{ color: "#1890ff" }} onClick={() => handleNavigation(text, "fiscalEntity")}>
            {text}
          </a>
        ),
      },
